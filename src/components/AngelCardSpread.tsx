...
const AngelCardSpread = ({ question, onError }: AngelCardSpreadProps) => {
  const [phase, setPhase] = useState<"idle" | "choose-count" | "shuffling" | "spread" | "loading" | "result">("idle");
  ...
  const handleShuffle = (count: number) => {
    setCardCount(count);
    setPhase("shuffling");
    const drawn = drawAngelCards(count);
    const labels = count === 1 ? ["Your Message"] : count === 2 ? ["Message 1", "Message 2"] : ["Past Guidance", "Present Wisdom", "Future Light"];
        ...
        <motion.div className="flex flex-col items-center gap-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          <div className="relative w-28 h-44">
            {[0, 1, 2].map((i) => (
              <motion.div
                key={i}
                className="absolute inset-0 rounded-lg overflow-hidden border border-accent/30"
